...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isAdmin
      ? 'https://72.167.51.48:8082/api/autenticacion/login-admin'
      : 'https://72.167.51.48:8082/api/autenticacion/login-estudiante';
    ...
    try {
      ...
      if (isAdmin) {
        const idResponse = await fetch(
          `https://72.167.51.48:8082/api/admin/id?username=${formData.username}`,
        ...
        const idResponse = await fetch(
          `https://72.167.51.48:8082/api/estudiantes/id-by-email?email=${formData.username}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        if (!idResponse.ok) {

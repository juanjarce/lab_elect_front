  ...
  const fetchPrestamos = async (page = 0) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://72.167.51.48:8082/api/admin/prestamos/prestados?page=${page}&size=5`,
        {
          headers: {
      ...
      if (response.data.status === 'Exito') {
          ...
          response.data.data.content.map(async (prestamo) => {
            try {
              // Obtener nombre
              const estudianteResponse = await axios.get(`https://72.167.51.48:8082/api/admin/estudiante/info?id=${prestamo.idEstudiante}`);
  ...
  const handleEntregaPrestamo = async (idPrestamo) => {
    try {
      ...
      const response = await axios.put(
        `https://72.167.51.48:8082/api/admin/prestamos/devolver/${idPrestamo}`,
        null,
        {
          headers: {
            Authorization: `Bearer ${token}`,

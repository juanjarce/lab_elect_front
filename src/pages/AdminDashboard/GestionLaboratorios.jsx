...
const GestionLaboratorios = () => {
  ...
  const cargarLaboratorios = async (page = 0, searchQuery = "") => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://labuq.catavento.co/services/api/admin/laboratorios/info?page=${currentPage}&size=${pageSize}&search=${search}`,
        {
          headers: {
  ...
  const eliminarLaboratorio = async () => {
    ...
    try {
      await axios.delete(
        `https://labuq.catavento.co/services/api/admin/laboratorios/eliminar/${laboratorioAEliminar.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },

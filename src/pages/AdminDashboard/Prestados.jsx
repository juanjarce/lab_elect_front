...
import PrestamoCardEntregado from "../cards/PrestamoCardEntregado";
  ...
  const fetchPrestamos = async (page = 0, searchQuery = "") => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://labuq.catavento.co/services/api/admin/prestamos/prestados?page=${page}&size=99&search=${searchQuery}`,
        {
          headers: {
          ...
          response.data.data.content.map(async (prestamo) => {
            try {
              const estudianteResponse = await axios.get(
                `https://labuq.catavento.co/services/api/admin/estudiante/info?id=${prestamo.idEstudiante}`,
              );
  ...
  const handleEntregaPrestamo = async (idPrestamo) => {
    try {
      ...
      const response = await axios.put(
        `https://labuq.catavento.co/services/api/admin/prestamos/devolver/${idPrestamo}/${id}`,
        null,
        {
          headers: {
            Authorization: `Bearer ${token}`,

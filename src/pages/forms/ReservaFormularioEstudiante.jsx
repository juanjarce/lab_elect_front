...
const ReservaFormularioEstudiante = ({ show, onClose, laboratorio }) => {
  ...
  /**
   * handles the load of the agenda
   * @returns
   */
  const cargarAgenda = async () => {
    ...
    try {
      const response = await axios.get(
        `https://labuq.catavento.co/services/api/estudiantes/agenda/${id}/${laboratorio.id}/${fecha}`,
        {
  ...
  const handleReservar = async (agendaId) => {
    ...
    try {
      const response = await axios.post(
        "https://labuq.catavento.co/services/api/estudiantes/reserva/reservar",
        {
          agendaId: agendaId,
          estudianteId: id,
        },
      ...
      setMensaje({
        type: "danger",
        text: error.response?.data?.message || "Error al realizar la reserva",
      });
    } finally {

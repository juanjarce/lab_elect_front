...
const ProductosPestaña = () => {
  ...
  const cargarProductos = async (page) => {
    ...
    try {
      const response = await axios.get(`https://72.167.51.48:8082/api/estudiantes/productos/paginated?page=${page}&size=${pageSize}`, {
  ...
  const obtenerCantidadDisponible = async (productoId) => {
    try {
      const response = await axios.get(`https://72.167.51.48:8082/api/estudiantes/productos/${productoId}/cantidad-disponible`);
  ...
  const handleAgregarCarrito = async (productoId, cantidad) => {
    ...
    try {
      await axios.post(`https://72.167.51.48:8082/api/estudiantes/producto/agregar/${id}/${productoId}?cantidad=${cantidad}`, null, {
        headers: { Authorization: `Bearer ${token}` },
      });

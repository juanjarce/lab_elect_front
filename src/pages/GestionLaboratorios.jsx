...
const GestionLaboratorios = () => {
  ...
  const eliminarLaboratorio = async () => {
    ...
    try {
        ...
        // Actualizar la lista eliminando el laboratorio borrado
        ...
        console.log(`Laboratorio con ID ${laboratorioAEliminar.id} eliminado correctamente.`);
    } catch (error) {
  ...
  return (
    <Container fluid>
      ...
      <Button variant="success" className="mb-3" onClick={() => setShowAgregarModal(true)}>
          ...
          {laboratoriosPorPagina.map(lab => (
            <CSSTransition key={lab.id} timeout={300} classNames="fade">
              <Col md={3} sm={6} xs={12} className="mb-3">
                <Card className="laboratorio-card fade-in" style={{ textAlign: 'center', padding: '10px' }}>
                  ...
                  <Card.Body>
                    ...
                    <Button variant="warning" onClick={() => handleEditar(lab)} className="mr-2">Editar</Button>
                    <Button variant="danger" onClick={() => confirmarEliminacion(lab)}>Eliminar</Button>
                  </Card.Body>
                </Card>
              </Col>
            </CSSTransition>
    ...
    <div className="d-flex justify-content-center mt-3">
      ...
      <AgregarLaboratorioForm show={showAgregarModal} onClose={() => setShowAgregarModal(false)} onSave={cargarLaboratorios} />
      <ModificarLaboratorioForm show={showModificarModal} onClose={() => setShowModificarModal(false)} onSave={cargarLaboratorios} laboratorio={laboratorioAEditar} />
      ...
      <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)}>
        ...
        <Modal.Body>
          ¿Estás seguro de que deseas eliminar el laboratorio "{laboratorioAEliminar?.nombre}"?
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowConfirmModal(false)}>Cancelar</Button>
          <Button variant="danger" onClick={eliminarLaboratorio}>Eliminar</Button>
        </Modal.Footer>
      </Modal>
    </Container>
  );
